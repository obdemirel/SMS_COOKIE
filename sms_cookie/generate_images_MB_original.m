function [img_grappa_sense1, img_grappa_phase_sens] = generate_images_MB(kspace_grappa_recons, kspace_recon_center)
...
%kspace_to_im = @(x) (fftshift(ifft2(x), 1));
...
Coil_sensitivites = repmat(im_coils ./ permute(repmat(rssq(im_coils)+eps,[1 1 1 no_c]),[1 2 4 3]), [1 1 1 1 no_ims]);
...
% a1 = squeeze(Coil_sensitivites(:,:,20,1,2));
% figure, subplot(1,3,1),imshow(angle(a1),[])
% a1 = squeeze(kspace_grappa_recons(:,:,20,1,2));
% subplot(1,3,2), imshow(angle(a1),[])
% subplot(1,3,3), imshow(abs(ifft2(a1)),[])
% drawnow()
% 
